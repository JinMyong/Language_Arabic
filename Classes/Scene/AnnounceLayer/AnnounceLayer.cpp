...
void AnnounceLayer::doNextCountDown()
{
    ...
    countLabel = CCLabelTTF::create(countString.c_str(), LETTER_NOR_FONT, 256 * SCALE);
#else
    // Do number, 3-2-1
    string countString;
    switch (countdown) {
    ...
    countLabel = CCLabelTTF::create(countString.c_str(), STANDARD_FONT, 256 * SCALE);
#endif
...
void AnnounceLayer::announceLetter(int letterIndex)
{
    ...
    if (letterLabel == NULL) {
        letterLabel = CCLabelTTF::create(letterString.c_str(), STANDARD_FONT, STD_HEADING_SIZE * SCALE * 1.5);
        // Locate the label
...
void AnnounceLayer::announceHit(bool hit)
{
    ...
    // Create and initialize a label
    hitLabel = CCLabelTTF::create(hitMissString.c_str(), STANDARD_FONT, 64 * SCALE);
