...
void ChooseLetterScene::createUI()
{
    ...
    CCSprite* title = CCSprite::create(getResPath(CHOOSE_TITLE_IMG));
    title->setScale(MIN(SCALEX, SCALEY));
    title->setPosition(ccp(winSize.width * 0.5, 1076 * SCALEY));
    this->addChild(title);
...
void ChooseLetterScene::createLetterBoard()
{
    ...
    if (fontType) {
        fontName = LETTER_NOR_FONT;
        factor = 1.2;
    }else
    {
        fontName = LETTER_CUR_FONT;
        factor = 1.2;
    }
...
//    const int yOffset = 450 * SCALEY;
    ...
    lastLetterLabel = CCLabelTTF::create(letterName.c_str(), STANDARD_FONT, 40 * SCALE);
