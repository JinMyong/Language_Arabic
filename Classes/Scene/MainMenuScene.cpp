...
void MainMenuScene::createUI()
{
    ...
    CCSprite* top_title = CCSprite::create(getResPath(MAINMENU_TOP_TITLE_IMG));
    top_title->setScale(SCALE);
    top_title->setPosition(ccp(winSize.width * 0.5, 1090 * SCALEY));
    this->addChild(top_title);
    
    // Add Bottom title ("Wish your Success...")
    CCSprite* bottom_title = CCSprite::create(getResPath(MAINMENU_BOTTOM_TITLE_IMG));
    bottom_title->setPosition(ccp(winSize.width * 0.5, 45 * SCALEY));
    bottom_title->setScale(SCALE);
